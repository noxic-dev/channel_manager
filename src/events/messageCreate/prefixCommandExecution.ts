...
import { type Message, type PermissionsBitField } from 'discord.js';
...
export default async (interaction: Message): Promise<unknown> => {
    ...
    const localCommand = commands.PrefixCommands.find(
        (c) => c.name === args[1]
    );
    console.log(args);
            ...
            switch (option.type) {
                ...
                case PrefixCommandsOptions.roleMentionbale:
                    if (!isValidOption(argValue, option.type)) {
                        ...
                        if (
                            option.type ===
                            PrefixCommandsOptions.userMentionable
                        ) {
                            ...
                            option.type ===
                            PrefixCommandsOptions.channelMentionable
                        ) {
                            ...
                            option.type ===
                            PrefixCommandsOptions.roleMentionbale
                        ) {
    ...
    Promise.resolve()
        .then(() => localCommand.callback(interaction, config, args))
